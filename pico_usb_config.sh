...
main() {
    ...
    log "Starting Luckfox Pico Plus USB configuration..."

    # Configure USB interface
    configure_usb_interface "$usb_iface"
    
    # Setup internet sharing if requested
    if [[ "$share_internet" == true ]] && [[ -n "$inet_iface" ]]; then
        setup_internet_sharing "$usb_iface" "$inet_iface"
    fi
    
    # Save persistent configuration
    save_persistent_config "$usb_iface"
    
    # Test connection
    test_connection
    
    # Show final status
    show_status "$usb_iface"
